...
A Python Telegram bot that bridges chat messages to local CLI agents like `codex`, `agy`, and `opencode`. It uses a headless request/response model, rendering provider replies as rich text while keeping system messages in plain text.

## Features

- **Multi-Provider Support**: Supports `codex`, `agy`, and `opencode` with per-chat logical sessions.
...
2. Edit the service file (update paths, `User`, `WorkingDirectory`, and ensure the `PATH` environment variable contains your virtual environment's bin folder and the directory of your local CLI agents like agy or opencode).
